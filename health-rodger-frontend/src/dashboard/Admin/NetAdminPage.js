...
function NetAdminPage() {
  ...
  const handleDeployPeer = async () => {
    ...
    try {
      const response = await axios.post(`${config.apiUrl}/create-peer`, { orgName: selectedOrg.name });
  ...
  const handleRemoveOrg = async (orgName) => {
    try {
      const response = await axios.post(`${config.apiUrl}/disable-organization`, { orgName });
